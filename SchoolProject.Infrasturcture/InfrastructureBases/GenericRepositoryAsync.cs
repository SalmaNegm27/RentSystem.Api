...
namespace RentSystem.Infrasturcture.InfrastructureBases
{
    public class GenericRepositoryAsync<T> : IGenericRepositoryAsync<T> where T : class
    {
        ...
        public async Task<IEnumerable<T>> FindAllByExpression(Expression<Func<T, bool>> match, Expression<Func<T, object>>[] includes = null,
       int? take = null, int? skip = null, Expression<Func<T, object>> orderBy = null, string orderByDirection = "ASC")
        ...
        public async Task<T> FindByExpression(Expression<Func<T, bool>> match, Expression<Func<T, object>>[] includes = null)
        {
            IQueryable<T> query = _dbContext.Set<T>().Where(match);
            if (includes != null)
            {
